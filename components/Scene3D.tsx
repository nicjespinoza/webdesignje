...
const NeuralNetwork = ({ count = 60, radius = 4 }) => {
  ...
  const connections = useMemo(() => {
    ...
    const threshold = 2.5;
    // Bolt Optimization: Precalculate threshold squared
    const thresholdSq = threshold * threshold;
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        // Bolt Optimization: skip redundant bidirectional pairs
        if (j > i) {
          // Bolt Optimization: avoid expensive Math.sqrt calls with distanceToSquared
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Bolt Optimization: avoid expensive Math.sqrt calls in high frequency useFrame
