...
const NeuralNetwork = ({ count = 60, radius = 4 }) => {
  ...
  const connections = useMemo(() => {
    ...
    const threshold = 2.5;
    // Optimization: hoist the squared threshold calculation and use distanceToSquared to avoid expensive Math.sqrt
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Optimization: use distanceToSquared (0.5 * 0.5 = 0.25) to avoid expensive Math.sqrt
