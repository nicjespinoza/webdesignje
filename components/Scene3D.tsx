  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // ⚡ Bolt: Use distanceToSquared to avoid expensive Math.sqrt calls in nested loop
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // ⚡ Bolt: Use distanceToSquared to avoid expensive Math.sqrt calls in animation loop (0.5^2 = 0.25)
