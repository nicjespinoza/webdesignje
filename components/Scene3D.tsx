  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        // Optimize: check j > i to avoid duplicate bidirectional pairs
        if (j > i) {
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Optimize: use distanceToSquared (0.5 * 0.5 = 0.25) to avoid expensive Math.sqrt calls in animation loop
