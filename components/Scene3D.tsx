...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    // ⚡ Bolt: Cache reusable objects to prevent garbage collection stutters in useFrame
    const { tempObj, tempDir } = useMemo(() => ({
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            // ⚡ Bolt: Reuse cached vector instead of allocating a new one
            tempDir.subVectors(agent.dest, agent.pos).normalize();
            agent.pos.add(tempDir.multiplyScalar(agent.speed));

            // ⚡ Bolt: Use distanceToSquared for fast rejection, avoiding expensive Math.sqrt
