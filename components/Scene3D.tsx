...
const NeuralNetwork = ({ count = 60, radius = 4 }) => {
  ...
  const connections = useMemo(() => {
    ...
    const threshold = 2.5;
    // Performance optimization: calculate squared threshold once
    const thresholdSq = threshold * threshold;

    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // Performance optimization: use squared distance to avoid expensive Math.sqrt calls in nested loops
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Performance optimization: use squared distance (0.5 * 0.5 = 0.25) to avoid expensive Math.sqrt calls in animation loop
