  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // ⚡ Bolt Optimization: Use distanceToSquared to avoid expensive Math.sqrt in nested loops
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // ⚡ Bolt Optimization: Use distanceToSquared to avoid expensive Math.sqrt in animation loop
