  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // Bolt ⚡ Optimization: Using distanceToSquared instead of distanceTo to avoid expensive Math.sqrt calls in nested loop
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Bolt ⚡ Optimization: distanceToSquared instead of distanceTo in high frequency animation loop
