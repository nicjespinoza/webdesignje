  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // Optimization: use distanceToSquared to avoid expensive Math.sqrt calls in O(n^2) loop
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Optimization: use distanceToSquared to avoid expensive Math.sqrt calls in high-frequency animation loop
