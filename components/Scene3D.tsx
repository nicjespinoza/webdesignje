  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // Optimization: Check squared distance to avoid Math.sqrt
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Optimization: check squared distance to avoid Math.sqrt calls
            // If close to destination, pick new destination
            if (agent.pos.distanceToSquared(agent.dest) < thresholdDistSq) {
                agent.dest.set(
                    (Math.random() - 0.5) * radius * 2,
                    (Math.random() - 0.5) * radius * 2,
                    (Math.random() - 0.5) * radius * 2
