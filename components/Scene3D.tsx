  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // ⚡ Bolt Optimization: Use distanceToSquared to avoid Math.sqrt in O(n^2) nested loop
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // ⚡ Bolt Optimization: Use distanceToSquared to avoid Math.sqrt in animation frame loop
