...
const NeuralNetwork = ({ count = 60, radius = 4 }) => {
  ...
  const connections = useMemo(() => {
    ...
    const threshold = 2.5;
    // Optimization: hoist threshold squared outside the loop and use distanceToSquared to avoid expensive Math.sqrt calls in O(n^2) operations
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Optimization: use distanceToSquared to avoid expensive Math.sqrt calculation in the high-frequency animation loop
