...
const NeuralNetwork = ({ count = 60, radius = 4 }) => {
  ...
  const connections = useMemo(() => {
    ...
    const threshold = 2.5;
    // Optimization: hoist the squared threshold calculation outside the loop
    const thresholdSq = threshold * threshold;
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // Optimization: use distanceToSquared to avoid expensive Math.sqrt inside O(n^2) loop
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Optimization: use distanceToSquared to avoid expensive Math.sqrt in high-frequency animation loop
