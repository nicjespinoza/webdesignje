  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        if (i !== j) {
          // Bolt: Use squared distance to avoid expensive Math.sqrt calls in O(n^2) nested loop
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    // Bolt: Cache tempDir to avoid per-frame allocations and garbage collection stutter
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            // Bolt: Reuse cached tempDir to avoid allocation and use distanceToSquared for fast rejection
