  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
      particles.forEach((p2, j) => {
        // Optimize O(N^2) loop: use j > i to avoid duplicate bidirectional pairs
        // and eliminate Math.sqrt by using distanceToSquared
        if (j > i) {
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    // Pre-allocate to prevent GC spikes in useFrame
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            // Reuse vector instead of allocating per frame
            tempDir.subVectors(agent.dest, agent.pos).normalize();
            agent.pos.add(tempDir.multiplyScalar(agent.speed));

            // Use distanceToSquared to avoid Math.sqrt calculation
            if (agent.pos.distanceToSquared(agent.dest) < 0.25) { // 0.5^2
                agent.dest.set(
                    (Math.random() - 0.5) * radius * 2,
                    (Math.random() - 0.5) * radius * 2,
                    (Math.random() - 0.5) * radius * 2
