...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    // Cache Vector3 outside of useFrame to prevent garbage collection stutters
    const cachedDir = useMemo(() => new THREE.Vector3(), []);
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            // Reuse cached vector to avoid new object creation per frame
            cachedDir.subVectors(agent.dest, agent.pos).normalize();
            agent.pos.add(cachedDir.multiplyScalar(agent.speed));
