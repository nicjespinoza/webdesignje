...
const NeuralNetwork = ({ count = 60, radius = 4 }) => {
  ...
  const connections = useMemo(() => {
    ...
    particles.forEach((p1, i) => {
        // Optimization: hoisted threshold calculation for squared distance
        const thresholdSq = threshold * threshold;
      particles.forEach((p2, j) => {
        if (i !== j) {
            // Optimization: use distanceToSquared to avoid expensive Math.sqrt in nested loop
...
const DataPulses = ({ radius }: { radius: number }) => {
    ...
    useFrame(() => {
        ...
        agents.forEach((agent, i) => {
            ...
            // Optimization: use distanceToSquared to avoid expensive Math.sqrt inside frame loop
