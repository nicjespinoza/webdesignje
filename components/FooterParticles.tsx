  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        // ⚡ Bolt Optimization: Pre-calculate squared distances to avoid Math.sqrt in O(n^2) nested loop
        ...
        const mouseDistanceSq = dxMouse * dxMouse + dyMouse * dyMouse;

        if (mouseDistanceSq < MOUSE_DISTANCE_SQ) {
            const distance = Math.sqrt(mouseDistanceSq);
            ctx.beginPath();
