    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i; j < particles.length; j++) {
            ...
            // ⚡ Bolt Optimization: Calculate squared distance to avoid Math.sqrt in hot loop
            const distSq = dx * dx + dy * dy;

            if (distSq < CONNECTION_DISTANCE * CONNECTION_DISTANCE) {
                // Only compute actual distance (Math.sqrt) if necessary for opacity calculation
        ...
        // ⚡ Bolt Optimization: Use squared distance for hit check
        const distSq = dx * dx + dy * dy;

        if (distSq < MOUSE_DISTANCE * MOUSE_DISTANCE) {
            // Compute actual distance for opacity only when in range
            const distance = Math.sqrt(distSq);
            ctx.beginPath();
