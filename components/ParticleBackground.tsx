...
const ParticleBackground: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const drawLines = () => {
            if (!ctx) return;
            // Optimization: Hoist invariant calculations out of O(n^2) loop
            const maxDist = 180;
            ...
            for (let i = 0; i < particles.length; i++) {
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    // Optimization: Use squared distance for initial threshold check
                    const distSq = dx * dx + dy * dy;

                    if (distSq < maxDistSq) {
                        const distance = Math.sqrt(distSq); // Only compute root if needed for opacity
