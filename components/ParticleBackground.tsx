...
const ParticleBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const updateParticle = (p: CanvasParticle, others: CanvasParticle[]) => {
      ...
      p.isHovered = distMouseSq < 1600; // Optimization: use squared distance (40^2)
      ...
      if (p.isHovered) {
        others.forEach(other => {
          ...
          if (distOtherSq < 14400) {
            // Optimization: use squared distance (120^2)
            other.x += dxo * 0.005;
            other.y += dyo * 0.005;
          }
        });
    ...
    const drawParticle = (p: CanvasParticle) => {
      ...
      ctx.fillStyle = p.isHovered
        ? `rgba(${COLORS.platinum}, 1)`
        : `rgba(${COLORS.platinum}, ${currentOpacity + 0.2})`;
      ctx.fill();
    };
