    ...
    useEffect(() => {
        ...
        class Particle {
            ...
            update(others: Particle[]) {
                ...
                // Optimization: Use squared distance to avoid expensive Math.sqrt calculation
                this.isHovered = distMouseSq < 1600; // 40 * 40 = 1600
                ...
                if (this.isHovered) {
                    ...
                    others.forEach(p => {
                        ...
                        // Optimization: Use squared distance to avoid expensive Math.sqrt calculation
                        if (distOtherSq < 14400) { // 120 * 120 = 14400
                            p.x += dxo * 0.005;
                            p.y += dyo * 0.005;
                        }
                    });
        ...
        const drawLines = () => {
            ...
            for (let i = 0; i < particles.length; i++) {
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    // Optimization: Use squared distance to avoid Math.sqrt calculation for all particle pairs
                    if (distSq < maxDistSq) {
                        const distance = Math.sqrt(distSq); // Only compute if within range
