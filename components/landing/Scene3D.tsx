  ...
  const connections = useMemo(() => {
    ...
    neurons.forEach((n1, i) => {
      neurons.forEach((n2, j) => {
        if (i !== j) {
          // ⚡ Bolt Optimization: Use distanceToSquared instead of distanceTo
          // This avoids the expensive Math.sqrt call in O(N^2) loop
          const distSq = n1.position.distanceToSquared(n2.position);
          // Different thresholds per layer (squared to match distSq)
...
const DataPulses = ({ radius, count }: { radius: number; count: number }) => {
  ...
  useFrame((state) => {
    ...
    agents.forEach((agent, i) => {
      ...
      // ⚡ Bolt Optimization: Use distanceToSquared instead of distanceTo
      // This avoids the expensive Math.sqrt call in the high-frequency useFrame render loop
