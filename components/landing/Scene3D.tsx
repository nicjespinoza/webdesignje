  ...
  const connections = useMemo(() => {
    ...
    neurons.forEach((n1, i) => {
      neurons.forEach((n2, j) => {
        if (i !== j) {
          // ⚡ Bolt: optimized distance calculation using distanceToSquared to avoid expensive Math.sqrt in nested loop
...
const DataPulses = ({ radius, count }: { radius: number; count: number }) => {
  ...
  useFrame((state) => {
    ...
    agents.forEach((agent, i) => {
      ...
      // ⚡ Bolt: optimized distance check using distanceToSquared to avoid Math.sqrt in high-frequency animation frame
