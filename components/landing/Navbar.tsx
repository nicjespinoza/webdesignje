...
const Navbar = ({
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'glass-panel py-3 shadow-lg backdrop-blur-md' : 'py-6 bg-transparent'}`}
    >
      <div className="container mx-auto px-6 flex justify-between items-center">
        <a href="#" className="flex items-center gap-3 group">
          <Logo isDark={isDark} size={64} />
          <div className="leading-none flex flex-col">
            <span className="text-2xl font-bold tracking-tight gradient-text transition hover:brightness-125 font-sans">
              Joseph Espinoza
            </span>
            <span className="text-xs font-medium tracking-[0.2em] text-slate-300 font-sans lowercase italic opacity-80">
              Web Design
            </span>
          </div>
        </a>
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden text-slate-300 p-2 rounded-lg hover:bg-white/5 transition-colors focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-[#C69320]"
          onClick={() => setIsOpen(!isOpen)}
          aria-label={t('nav.toggle_menu', { lng: lang })}
          title={t('nav.toggle_menu', { lng: lang })}
