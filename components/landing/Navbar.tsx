  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'glass-panel py-2 md:py-3 shadow-lg backdrop-blur-md' : 'py-3 md:py-6 bg-transparent'}`}>
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-1.5 p-1 glass-panel rounded-full border border-white/5 bg-white/5 shadow-inner">
            {languages.map((l) => (
              <button
                ...
                aria-label={`Change language to ${languageConfig[l].name}`}
                className={`flex items-center gap-2.5 px-3 py-1.5 rounded-full transition-all duration-300 relative group focus-visible:ring-2 focus-visible:ring-[#FBE18D] focus-visible:outline-none ${
                  lang === l 
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden text-slate-300 p-2 focus-visible:ring-2 focus-visible:ring-[#FBE18D] focus-visible:outline-none rounded-lg"
              ...
              <div className="grid grid-cols-2 gap-3">
                {languages.map((l) => (
                  <button
                    ...
                    aria-label={`Change language to ${languageConfig[l].name}`}
                    className={`flex items-center bg-white/5 gap-3 px-4 py-3 border border-white/10 rounded-xl text-sm font-medium transition-all focus-visible:ring-2 focus-visible:ring-[#FBE18D] focus-visible:outline-none ${lang === l ? 'border-[#C69320] text-[#FBE18D] bg-[#FBE18D]/5' : 'text-slate-300'}`}
                  >
                    <Image 
                      src={`https://flagcdn.com/w40/${languageConfig[l].flagCode}.png`} 
                      alt=""
