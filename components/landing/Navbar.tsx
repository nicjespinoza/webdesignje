  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-1.5 p-1 glass-panel rounded-full border border-white/5 bg-white/5 shadow-inner">
            {languages.map(l => (
              <button
                ...
                aria-pressed={lang === l}
                className={`flex items-center gap-2.5 px-3 py-1.5 rounded-full transition-all duration-300 relative group outline-none focus-visible:ring-2 focus-visible:ring-[#FBE18D] ${
                  lang === l
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden text-slate-300 p-2 rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-[#C69320] transition-shadow"
          onClick={() => setIsOpen(!isOpen)}
          aria-expanded={isOpen}
          aria-label={
            isOpen
              ? t('nav.closeMenu', { defaultValue: 'Cerrar menú', lng: lang })
              : t('nav.openMenu', { defaultValue: 'Abrir menú', lng: lang })
          }
        >
              ...
              <div className="grid grid-cols-2 gap-3">
                {languages.map(l => (
                  <button
                    ...
                    aria-pressed={lang === l}
                    className={`flex items-center bg-white/5 gap-3 px-4 py-3 border border-white/10 rounded-xl text-sm font-medium transition-all outline-none focus-visible:ring-2 focus-visible:ring-[#C69320] ${lang === l ? 'border-[#C69320] text-[#FBE18D] bg-[#FBE18D]/5' : 'text-slate-300'}`}
                  >
                    <Image
                      src={`https://flagcdn.com/w40/${languageConfig[l].flagCode}.png`}
                      alt=""
