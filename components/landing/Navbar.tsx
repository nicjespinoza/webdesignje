  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'glass-panel py-2 md:py-3 shadow-lg backdrop-blur-md' : 'py-3 md:py-6 bg-transparent'}`}>
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-1.5 p-1 glass-panel rounded-full border border-white/5 bg-white/5 shadow-inner">
            {languages.map((l) => (
              <button
                ...
                aria-pressed={lang === l}
                aria-label={`${t('nav.change_lang', { lng: lang })} ${languageConfig[l].name}`}
                className={`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C69320] flex items-center gap-2.5 px-3 py-1.5 rounded-full transition-all duration-300 relative group ${
                  lang === l 
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden text-slate-300 p-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C69320] rounded-md"
          onClick={() => setIsOpen(!isOpen)}
          aria-expanded={isOpen}
          aria-controls="mobile-menu"
          aria-label={isOpen ? t('nav.close_menu', { lng: lang }) : t('nav.open_menu', { lng: lang })}
        >
              ...
              <div className="grid grid-cols-2 gap-3">
                {languages.map((l) => (
                  <button
                    ...
                    aria-pressed={lang === l}
                    aria-label={`${t('nav.change_lang', { lng: lang })} ${languageConfig[l].name}`}
                    className={`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C69320] flex items-center bg-white/5 gap-3 px-4 py-3 border border-white/10 rounded-xl text-sm font-medium transition-all ${lang === l ? 'border-[#C69320] text-[#FBE18D] bg-[#FBE18D]/5' : 'text-slate-300'}`}
                  >
                    <Image 
                      src={`https://flagcdn.com/w40/${languageConfig[l].flagCode}.png`} 
                      alt=""
