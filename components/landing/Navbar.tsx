  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'glass-panel py-2 md:py-3 shadow-lg backdrop-blur-md' : 'py-3 md:py-6 bg-transparent'}`}>
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden text-slate-300 p-2 rounded-lg hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FBE18D] transition-colors"
          onClick={() => setIsOpen(!isOpen)}
          aria-expanded={isOpen}
          aria-controls="mobile-menu"
          aria-label={isOpen ? t('nav.closeMenu', { defaultValue: 'Cerrar menú', lng: lang }) : t('nav.openMenu', { defaultValue: 'Abrir menú', lng: lang })}
        >
