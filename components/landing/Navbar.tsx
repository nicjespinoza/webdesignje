  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'glass-panel py-3 shadow-lg backdrop-blur-md' : 'py-6 bg-transparent'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden text-slate-300 p-2 rounded-lg hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C69320]"
          onClick={() => setIsOpen(!isOpen)}
          aria-expanded={isOpen}
          aria-controls="mobile-menu"
          aria-label={t(isOpen ? 'Close menu' : 'Open menu', { lng: lang }) || (isOpen ? 'Cerrar menú' : 'Abrir menú')}
          title={t(isOpen ? 'Close menu' : 'Open menu', { lng: lang }) || (isOpen ? 'Cerrar menú' : 'Abrir menú')}
        >
