      ...
      <div className="container mx-auto px-6">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[#C69320] bg-[#FBE18D]/10 text-[#FBE18D] text-xs font-bold mb-6">
            <User size={14} /> Sobre Mí
          </div>
          <h2 className="text-4xl md:text-6xl font-bold mb-6 text-white text-center">Sobre <span className="gradient-text">Mí</span></h2>
          <p className="text-xl text-[#FBE18D] mb-6 font-medium text-center max-w-2xl">Desarrollador apasionado creando experiencias digitales con precisión y creatividad</p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <motion.div variants={fadeInUp} className="liquid-gold-card">
              <div className="liquid-gold-content">
                ...
                <p className="text-slate-300 mb-4">
                  Desarrollador full-stack especializado en tecnologías web modernas y soluciones potenciadas con IA. 
                  Apasionado por crear experiencias de usuario excepcionales y sistemas backend robustos.
                </p>
                <p className="text-slate-300">
                  Más allá del código, soy un arquitecto digital obsesionado con la precisión. Mi viaje comenzó no solo con la sintaxis,
                  sino con el deseo de construir sistemas que se sientan <span className="text-white font-medium border-b border-[#FBE18D]/30 pb-0.5">vivos</span>.
                </p>
                <p className="text-slate-300 mt-4">
                  ...
                  Mi filosofía es simple: <span className="text-[#FBE18D]/90">El rendimiento es el máximo lujo.</span>
                </p>
              </div>
            </motion.div>
            ...
            >
              {[
                { icon: Briefcase, title: "13+ Proyectos", subtitle: "Entregados" },
                { icon: Terminal, title: "Full Stack", subtitle: "Control End-to-End" },
                { icon: User, title: "Liderazgo", subtitle: "Mentoria de Equipos" }
              ].map((item) => (
                <motion.div
                  key={item.title}
                  variants={fadeInUp}
        ...
        >
          {[
            { label: 'Seguridad de Tipos', val: '100%', color: 'text-[#FBE18D]' },
            { label: 'Rendimiento', val: '98/100', color: 'text-[#FBE18D]' },
            { label: 'Tiempo en Línea', val: '99.9%', color: 'text-[#FBE18D]' },
            { label: 'Satisfacción', val: '100%', color: 'text-[#FBE18D]' }
          ].map((stat, index) => (
            <motion.div
              key={stat.label}
              variants={fadeInUp}
