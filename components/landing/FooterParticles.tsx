  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        // ⚡ Bolt: skip self-comparison (j = i + 1)
        for (let j = i + 1; j < particles.length; j++) {
          const dx = particles[i].x - particles[j].x;
          const dy = particles[i].y - particles[j].y;
          // ⚡ Bolt: replace Math.sqrt with squared distance check for fast rejection
        ...
        // ⚡ Bolt: replace Math.sqrt with squared distance check
        const distSq = dx * dx + dy * dy;
