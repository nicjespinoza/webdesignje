  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        // ⚡ Bolt: optimized O(N^2) loop by starting at i + 1 to avoid duplicate/self connections
        for (let j = i + 1; j < particles.length; j++) {
          const dx = particles[i].x - particles[j].x;
          const dy = particles[i].y - particles[j].y;
          // ⚡ Bolt: optimized distance calc using squared distance instead of Math.sqrt
        ...
        // ⚡ Bolt: optimized mouse distance calc using squared distance
        const distSq = dx * dx + dy * dy;
